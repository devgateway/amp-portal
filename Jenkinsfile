        ...
        if (imageIds.equals("")) { //If not found we build
          ...
          sh 'node -v'
                try {
                    ...
                    sh "docker build -q -t phosphorus.migrated.devgateway.org:5000/amppp-wp:${tag} --build-arg --build-arg AMPPP_PULL_REQUEST='${pr}' --build-arg AMPPP_BRANCH='${branch}' --label git-hash='${hash}' amppp-wp"
